...
class tfAugmentor(object):
    ...
    def __init__(self, tensor_list, label=[]):
        ...
        self.tensor_size = tf.shape(list(self.tensors.values())[0])
    ...
    def flip_left_right(self, prob):
        funcs = {}
        for k, type in self.types.items():
    ...
    def random_rotate(self, prob):
        funcs = {}
        angle = tf.random_uniform([self.tensor_size[0]], 0, 2*math.pi)
        for k, type in self.types.items():
            if type == "label":
    ...
    def elastic_deform(self, prob, strength, scale):
        funcs = {}
        dx = tf.random_uniform([self.tensor_size[0],
                                tf.floordiv(self.tensor_size[1], scale),
                                tf.floordiv(self.tensor_size[2], scale),
                                1], -1, 1)
        dy = tf.random_uniform([self.tensor_size[0],
                                tf.floordiv(self.tensor_size[1], scale),
                                tf.floordiv(self.tensor_size[2], scale),
                                1], -1, 1)
        dx = gaussian(dx, 0, 5)
        ...
        flow = tf.image.resize_bilinear(flow, self.tensor_size[1:3])
        for k, type in self.types.items():
            if type == "label":
...
if __name__ == "__main__":
    ...
    out = aug.elastic_deform(0.5, 200, 20)
